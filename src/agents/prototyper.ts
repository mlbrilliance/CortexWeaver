import { Agent, AgentConfig, TaskResult, TaskContext } from '../agent';
...
export class PrototyperAgent extends Agent {
  ...
  public get role(): string {
    return this.config?.role || 'Prototyper';
  }

  public get capabilities(): string[] {
    return this.config?.capabilities || ['pseudocode-generation', 'logic-flow-design', 'mermaid-diagrams'];
  }

  async initialize(config: AgentConfig): Promise<void> {
    config.role = 'Prototyper';
    config.capabilities = ['pseudocode-generation', 'logic-flow-design', 'mermaid-diagrams'];
    await super.initialize(config);
  }

  getPromptTemplate(): string {
    return `You are a senior implementation planner. Generate detailed pseudocode and logic flows before implementation.`;
  }
  ...
  async executeTask(): Promise<any> {
    ...
    return {
      success: true,
      result: 'Prototyper task completed',
      metadata: { agent: 'Prototyper' }
    };
  }
  ...
  async processContract(contractId: string): Promise<PrototypeResult> {
    try {
      if (!this.cognitiveCanvas) {
        throw new Error('Agent not initialized. Call initialize(config) first.');
      }
